...
bool CPU_ON = false;
printf(" CPU compute is %s\n\n", CPU_ON ? "ture":"false");
StopWatchInterface *timer = 0;
    // pick GPU
    findCudaDevice(argc, (const char **)argv);

    // find images
    const char *const sourceFrameName = "1.ppm";
    const char *const targetFrameName = "2.ppm";
...
//go to the data file firstly, and then run
// ~/home/wine/4d-seg/build/hso 
//it will generate 826 .flo files.

for (int n = 1; n < 2; n++){
char srcName[1024];
char tarName[1024];
sprintf(srcName, "%d.ppm", n);
sprintf(tarName, "%d.ppm", n+1);

    if (!LoadImageAsFP32(h_source, width, height, stride, srcName, argv[0]))
    {
        exit(EXIT_FAILURE);
    }

    if (!LoadImageAsFP32(h_target, width, height, stride, tarName, argv[0]))
    {
        exit(EXIT_FAILURE);
    }
...
sdkDeleteTimer(&timer);
    // comoupare results (L1 norm)
if(CPU_ON)
   bool status = CompareWithGold(width, height, stride, h_uGold, h_vGold, h_u, h_v);

char gpuflo[1024];
char cpuflo[1024];
sprintf(gpuflo, "%d_gpu.flo", n);
sprintf(cpuflo, "%d_cpu.flo", n);

    WriteFloFile(gpuflo, width, height, stride, h_u, h_v);

if(CPU_ON)
    WriteFloFile(cpuflo, width, height, stride, h_uGold, h_vGold);
